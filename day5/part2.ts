...
function applyInstructions9001(stacks: Stacks, instructions: Instructions): void {
    // run through the instructions and adjust the stacks accordingly
    // note that in the 9001 model, you have to pop multiple stacks "in order"
    for (const instruction of instructions) {
        const { count, from, to } = instruction;
        const fromLength = stacks[from].length;
        const removedElements = stacks[from].splice(fromLength - count, count);
        stacks[to].push(...removedElements);
    }
}
