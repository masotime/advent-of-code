...
function solve(maze: Maze): Path {
    ...
    // instead of using a "start position" for part 2, all elevations of a are "start positions",
    // and get pre-inserted into the paths array. We also mark all these "start psitions" as explored
    const rows = maze.grid.length;
    const cols = maze.grid[0].length;

    for (let r = 0; r < rows; r += 1) {
        for (let c = 0; c < cols; c += 1) {            
            const height = maze.grid[r][c];
            if (height === 0) {
                paths.push([{
                    location: { row: r, col: c},
                    direction: undefined
                }]);
                explored[r] = explored[r] || [];
                explored[r][c] = true;
            }
