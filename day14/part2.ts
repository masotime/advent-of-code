...
function dropSand(cave: Cave): Cave {
    ...
    function canMove(dir: Direction, sand: Coordinate): { result: boolean, location: Coordinate } {
        ...
        // added rule - now sand cannot move if location.row === cave.row.max + 1
        const blockedByFloor = sand.row === cave.row.max + 1;

        return {
            result: !blockedByFloor && cave.grid[location.row]?.[location.col] === undefined,
            location
        }
    }

    // new rule - stop falling once the source is blocked
    function sourceBlocked(sand: Coordinate) {
        return sand.col === 500 && sand.row === 0;
    }
    ...
    while (falling) {     
        ...
        if (!falling && sourceBlocked(sand)) {
            cave.sourceBlocked = true;

            // also mark the source with sand
            cave.sandCount += 1;
            cave.grid[0] = cave.grid[0] || [];
            cave.grid[0][500] = 'o';
            return cave;
        }
    }
    ...
    // we also need to update potential col boundaries
    cave.col.max = Math.max(cave.col.max, sand.col);
    cave.col.min = Math.min(cave.col.min, sand.col);
...
function renderCave(cave: Cave): string {
    let caveRender = '';
    // adjustments to for loop boundaries that the infinite floor is visible
    // * row ends at cave.row.max + 2
    // * col starts at cave.col.min - 2 and ends at cave.col.max + 2
    // * we force render a "floor" if we're at row === cave.row.max + 2
    for (let row = cave.row.min; row <= cave.row.max + 2; row += 1) {
        for (let col = cave.col.min - 2; col <= cave.col.max + 2; col += 1) {            
            const isInfiniteFloor = row === cave.row.max + 2;
            const object = isInfiniteFloor ? '#' : (cave.grid[row] ?? [])[col];
            caveRender += object || '.';
        }
        caveRender += '\n';
    }
...
export default function() {
    ...
    while (!cave.sourceBlocked) {
        dropSand(cave);
        // WARNING: Runs out of heap memory if I try to render every step
        // render = renderCave(cave);
        // console.log(render);
    }

    console.log(renderCave(cave));
